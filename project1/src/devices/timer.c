...
{
  //replace everything in timer_sleep() in timer.c with a not busy-waitting function. #timer.c
  ASSERT (intr_get_level () == INTR_ON);
  if(ticks < 0) return;       //if we use ASSERT(ticks > 0), we cannot pass the alarm-negative test. NULL is defined as 0 in pintos, if return NULL, there is a warning
  sema_down(&sema_);
  thread_current()->unblocked_ticks = timer_ticks() + ticks;        //unblocked_ticks is the time when we should wake the threads up
  sema_up(&sema_);
  put_thread_into_block_list();       //put the thread which will be blocked soon into the blocked list
}
